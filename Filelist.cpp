...
static void list_files_qwise(void)
{
   ...
   while (ftemp != NULL) {
      ...
      if (first_line  ||  _strcmpi(prev_ext, ftemp->ext) != 0) {
         if (first_line)
            first_line = 0 ;
         else 
            ncrlf() ;
         sprintf(tempstr, "%-*s: ", (int) maxext, ftemp->ext) ;
      ...
      if (col + slen > width) {
         if (!n.lfn_off) {
            nputs((ftemp->dirflag == 'd') ? n.colordir : ftemp->color, ", ") ;
         }
         ncrlf() ;
         sprintf(tempstr, "%*s  ", (int) maxext, " ") ;
         nputs(ftemp->color, tempstr) ;
         col = maxext+2 ;
         new_line = 1 ;
      } 
      ...
      nputs((ftemp->dirflag == 'd') ? n.colordir : ftemp->color, tempstr) ;
      col += slen ;
...
void put_disk_summary(void)
{
   ...
   nputs(n.colorxhead, itemp64.putstr()) ;
...
static void fileend(void)
{
   ...
   while (ftemp != NULL) {
      ...
      if ((ftemp->fsize % clbytes) > 0)  clusters++ ;
